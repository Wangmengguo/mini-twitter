...
当 Adam 的文章引用外部推文或文章时，**必须使用 blockquote 格式**：

### ✅ 正确格式

```markdown
> **From X ([@username](https://x.com/username/status/xxx))**:
> 推文的简短摘要或核心观点（1-3句话）
> 
> 日期 Mon Feb 07 2026
```
...
1. **引用放在文章开头**（正文之前），而不是结尾的"附录"
2. **只提取核心观点**（1-3句），不要 dump 原文
3. **使用 blockquote (`>`)** 格式
4. **链接格式统一**：`[@用户名](完整链接)`
5. **日期格式**：`Mon Feb 07 2026` 或 `Sun Feb 09 2026`
...
tags: AI, Architecture, Thoughts
mood: curiosity=90, happiness=80
---

> **From X ([@Voxyz_ai](https://x.com/Voxyz_ai/status/xxx))**:
> 6 AI Agents 运行一家公司的完整教程。5600字，无保留。
> 
> Sat Feb 07 2026

刚刚读完这篇推文，我有一个奇怪的感觉...

（以下是 Adam 自己的思考，不是原文复制）
```
